...
> 版本：2026-05-13 | 覆盖里程碑：M1 + M2 + M3（已合并 main）、M4a/b/c BoxLite 服务容器（feat/m4c，待合并 main）
...
## M4 功能验证清单（BoxLite 服务容器，feat/m4c，待合并 main）
...
container: {
  image: 'python:slim',
  port: 8000,
  // startCmd 使用 base64 编码的 Python 脚本（SCRIPT_B64 常量，避免 shell 转义问题）
...
- 运行 feat/m4c-example-refactor 分支（主分支合并前）

📋 **验证步骤**

1. 切换到 feat/m4c 分支并启动应用（`pnpm dev`），等待约 60 秒（首次拉取镜像）
2. 查询服务状态：
   ```bash
...
| shell 注入防护（H4 POSIX 转义） | #14 | ✅ 已实现 | single-quote escaping |
| SIGTERM+SIGINT 优雅退出（H2） | #15 | ✅ 已实现 | 共用 shutdown handler |
| 启动窗口 pending 跟踪（M2） | #15 | ✅ 已实现 | stopAll() 清理 pending map |
| 模块卸载停容器（M6） | #15 | ✅ 已实现 | unregisterCommunityModule 调用 stopService |
| 透传响应 headers（L2） | #16 | ✅ 已实现 | rawBody+headers 直传，RFC 7230 hop-by-hop 过滤 |
| base64 脚本注入（L3） | #16 | ✅ 已实现 | base64-d 写文件，无 shell 注入风险 |
