...
> 版本：2026-05-11 | 覆盖里程碑：M1 + M2（已实现）、M3（规划）
...
> 以下功能均处于**规划阶段**，尚未在当前代码库中实现。
> 此章节供技术 Review 使用，帮助评估实现方案的可行性和验收标准。

### M3 核心功能规划总览

| # | 功能 | 实现位置（预期） | 优先级 |
|---|------|----------------|--------|
| 1 | 模块从 npm 安装（`ModuleManagerAPI.install(packageName)`） | `src/main/backend-manager.ts` + 新增 npm runner | 高 |
| 2 | 安装后自动 register + sidebar 即时反映 | `src/backend/capability-registry.ts` + `src/renderer/App.tsx` | 高 |
| 3 | 卸载流程 + 失败回滚 | `src/main/backend-manager.ts`（`uninstall(id)`） | 高 |
| 4 | 应用产品名从 Agent24-Desktop 改为 Agent24 | `package.json`、`electron-builder.yml`、`src/renderer/App.tsx` | 中 |
| 5 | `@auraaihq/memory` 升级到 L0-L3 分层 | `src/backend/` 新增 memory 模块 | 中 |
| 6 | ATIF 轨迹采集 | Conversation Layer 新增 archive 写入 | 低 |
| 7 | Agent24 CLI → npm 包迁移 | 独立包 `@auraaihq/cli` | 低 |

---

### M3 详细规划

#### 3.1 模块从 npm 安装（`ModuleManagerAPI.install(packageName)`）

**功能描述**

用户在 ModulesManager UI 输入 npm 包名（如 `@auraaihq/module-wechat`），点击安装，
主进程通过独立 child process 执行 `npm install <packageName>`，安装到
`~/.agent24/modules/` 目录，完成后将模块 require 并注册到 capability-registry。

**预期实现位置**

- `src/main/backend-manager.ts`：新增 `install(packageName: string): Promise<InstallResult>` 方法，内部 `spawn('npm', ['install', packageName, '--prefix', modulesDir])`
- `src/main/ipc/index.ts`：新增 `modules:install` IPC channel
- `src/backend/capability-registry.ts`：新增运行时动态 `require()` + `register()` 逻辑
- `src/renderer/pages/`（新增）：ModulesManager 安装面板，输入框 + 进度条

**验收标准（实现后验证方式）**

1. 在 ModulesManager UI 输入合法 npm 包名，点击「安装」
2. 安装过程中显示进度状态（`Installing…`）
3. 安装成功后，`/api/modules` 立即返回新模块的 manifest
4. 若新模块类型为 `ui` 且有 `navItem`，sidebar 5 秒内出现新导航项（无需重启）
5. 安装失败时 UI 显示错误信息，`~/.agent24/modules/` 无残留文件

---

#### 3.2 安装后自动 Register + Sidebar 即时反映

**功能描述**

npm install 完成后，后端 daemon 无需重启即可加载新模块（热注册）。
前端 `App.tsx` 的 5 秒轮询机制已就位（`modulesList()` + `setModules()`），
只需后端 `/api/modules` 返回更新后的模块列表即可驱动 UI 更新。

**预期实现位置**

- `src/backend/capability-registry.ts`：将静态数组 `MODULES` 改为动态注册表，支持运行时 `add(module)` 操作
- `src/backend/server.ts`：确保 `/api/modules` 每次请求都读取最新注册表（当前已是动态读取 `MODULES`）
- `src/renderer/App.tsx`：现有 5 秒轮询逻辑已能驱动 sidebar 更新，无需改动

**验收标准（实现后验证方式）**

1. 后端 daemon 在不重启的情况下，`/api/modules` 响应包含新安装的模块
2. 前端 sidebar 在 5 秒内（下次轮询）自动出现新模块导航项
3. 反复安装/卸载，模块列表与 sidebar 始终保持同步，无重复项
...
用户从 ModulesManager UI 点击「卸载」某模块：
1. 主进程先将模块从 capability-registry 注销（`unregister(id)`）
2. 执行 `npm uninstall <packageName>` 移除文件
3. 若 npm 命令失败，回滚注册（重新 `register()`）并向 UI 报告错误
4. 成功后 sidebar 即时移除对应导航项

**预期实现位置**

- `src/main/backend-manager.ts`：`uninstall(id: string): Promise<UninstallResult>`
- `src/main/ipc/index.ts`：新增 `modules:uninstall` IPC channel
- `src/backend/capability-registry.ts`：新增 `remove(id)` + 回滚机制

**验收标准（实现后验证方式）**

1. 点击「卸载」后，`/api/modules` 不再返回该模块 manifest
2. Sidebar 5 秒内移除对应导航项
3. 模拟卸载失败（只读文件系统）：模块依然可访问，UI 显示回滚成功提示
4. `~/.agent24/modules/` 目录下对应包目录已删除（成功路径）

---

#### 3.4 应用产品名从 Agent24-Desktop 改为 Agent24

**功能描述**

随着 M3 里程碑完成，应用去掉「Desktop」后缀，为未来 mobile 端预留命名空间。
涉及：仓库 rename（GitHub 层面）、`package.json` 中 `productName` 字段、
`electron-builder` 配置、renderer 中的 logo 文字、macOS bundle ID 等。

**预期实现位置**

- `package.json`：`"productName": "Agent24"`，`"name": "agent24-desktop"` → `"agent24"`
- `electron-builder.yml`（如存在）：`productName`、`appId`
- `src/renderer/App.tsx` 第 109 行：`<div className="sidebar-logo-text">Agent24-Desktop` → `Agent24`
- macOS `Info.plist`（构建时自动生成）：`CFBundleName`

**验收标准（实现后验证方式）**

1. `pnpm build` 打包后，macOS `.app` 文件名为 `Agent24.app`（而非 `Agent24-Desktop.app`）
2. 应用启动后，sidebar logo 区显示 `Agent24`（无「Desktop」后缀）
3. macOS Dock 悬停提示和 Finder 中应用名均显示 `Agent24`
4. `app.getName()` IPC 调用返回 `"Agent24"`
...
| 功能 | 实现文件 | 验收方式 | 预期里程碑 |
|------|---------|---------|-----------|
| `ModuleManagerAPI.install(pkg)` | `backend-manager.ts`、`ipc/index.ts` | npm 包从 UI 一键安装，daemon 不重启即可调用 | M3 中期 |
| 安装后自动 register + sidebar 更新 | `capability-registry.ts`、`App.tsx`（已就位） | 安装完成 5 秒内 sidebar 出现新导航项 | M3 中期 |
| `ModuleManagerAPI.uninstall(id)` + 回滚 | `backend-manager.ts`、`capability-registry.ts` | 卸载后模块消失；失败则回滚可调用 | M3 中期 |
| 产品名改为 Agent24 | `package.json`、`App.tsx`、builder config | `.app` 文件名和 UI 均显示「Agent24」 | M3 末期 |
| `@auraaihq/memory` L0-L3 升级 | `src/backend/` 新增 memory 模块 | L0（in-memory）、L1（SQLite）、L2/L3 分层可配置 | M3 后期 |
| Agent24 CLI npm 包迁移 | 独立 `@auraaihq/cli` 包 | `npx @auraaihq/cli install <skill>` 替代旧 `install.sh` | M3 后期 |
| ATIF 轨迹采集 | Conversation Layer + archive writer | 对话轨迹写入本地 archive，可供 evolver 读取 | M3 后期 |
